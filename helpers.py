...
def evaluate(model, X, P, Y_one_hot, name, thresh_cam=0.5, thresh_iou=0.5, flag_preprocess=True):
    cam_list = model.calc_cam(X, Y_one_hot, flag_preprocess=flag_preprocess)
    bbox_list = model.location(X=X, Y=Y_one_hot, thresh=thresh_cam)
